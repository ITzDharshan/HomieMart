<?php
session_start();
include 'db_connection.php'; // Ensure this is correct
...
if (!isset($_SESSION['business_type']) || $_SESSION['business_type'] != 'Art and Painting') {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
            ...
            if (isset($_FILES['productImage']) && $_FILES['productImage']['error'] == 0) {
                ...
                if (move_uploaded_file($imageTmpName, $imageFolder)) {
                    ...
                    if ($conn->query($sql) === TRUE) {
                        echo "<script>alert('Product added successfully!'); window.location.href='AP_product.php';</script>";
                        exit();
                    } else {
                        echo "<script>alert('DB Error: " . addslashes($conn->error) . "'); window.location.href='AP_product.php';</script>";
                        exit();
                    }
                } else {
                    echo "<script>alert('Failed to upload image.'); window.location.href='AP_product.php';</script>";
                    exit();
                }
            } else {
                echo "<script>alert('Please upload a valid image.'); window.location.href='AP_product.php';</script>";
                exit();
            }
        }
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['productId'])) {
            ...
            if (mysqli_query($conn, $update)) {
                echo "<script>alert('Product updated successfully!'); window.location.href='AP_product.php';</script>";
            } else {
...
if (isset($_GET['delete'])) {
    ...
    if ($delete_result) {
        echo "<script>alert('Product deleted successfully!'); window.location.href='AP_product.php';</script>";
    } else {
...
<body>
	...
	<section id="sidebar">
		...
		<ul class="side-menu top">
			...
			<li class="active">
				<a href="AP_product.php">
					<i class='bx bx-cart-alt'></i>
					<span class="text">Products</span>
				</a>
			</li>
			<li>
				<a href="AP_message.php">
	...
	<section id="content">
		...
		<main>
			<div class="head-title">
				<div class="left">
					<h1>Artist / Painters View Products</h1>
				</div>
			</div>
			...
			<div class="order">
				...
				<table>
					...
					<tbody>
						...
						$query = "SELECT * FROM products WHERE homemaker_email = '$userEmail'";
						...
						if ($view_products && mysqli_num_rows($view_products) > 0) {
							while ($row = mysqli_fetch_assoc($view_products)) {
								...
								echo "<p>{$name}</p>";
								echo "<small>By: {$homemaker_name} ({$homemaker_email})</small>";
								echo "</td>";

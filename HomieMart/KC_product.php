...
if (!isset($_SESSION['business_type']) || $_SESSION['business_type'] != 'Knitting and Crochet') {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
            ...
            if (isset($_FILES['productImage']) && $_FILES['productImage']['error'] == 0) {
                ...
                if (move_uploaded_file($imageTmpName, $imageFolder)) {
                    ...
                    if ($conn->query($sql) === TRUE) {
                        echo "<script>alert('Product added successfully!'); window.location.href='KC_product.php';</script>";
                        exit();
                    } else {
                        echo "<script>alert('DB Error: " . addslashes($conn->error) . "'); window.location.href='KC_product.php';</script>";
                        exit();
                    }
                } else {
                    echo "<script>alert('Failed to upload image.'); window.location.href='KC_product.php';</script>";
                    exit();
                }
            } else {
                echo "<script>alert('Please upload a valid image.'); window.location.href='KC_product.php';</script>";
                exit();
            }
        }
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['productId'])) {
            ...
            if (mysqli_query($conn, $update)) {
                echo "<script>alert('Product updated successfully!'); window.location.href='KC_product.php';</script>";
            } else {
...
if (isset($_GET['delete'])) {
    ...
    if ($delete_result) {
        echo "<script>alert('Product deleted successfully!'); window.location.href='KC_product.php';</script>";
    } else {
...
<body>
	...
	<section id="sidebar">
		...
		<ul class="side-menu top">
			...
			<li class="active">
				<a href="KC_product.php">
					<i class='bx bx-cart-alt'></i>
					<span class="text">Products</span>
				</a>
			</li>
			<li>
				<a href="KC_message.php">

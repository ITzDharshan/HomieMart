...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $delete_query = "DELETE FROM admincontact WHERE id = $id";
							...
							<?php
								...
								if (mysqli_num_rows($view_messages) > 0) {
									...
									while ($row = mysqli_fetch_assoc($view_messages)) {
										...
										echo "<td>{$subject}</td>";
										echo "<td>{$message}</td>";
										echo "<td><span class='reply'>Respond</span></td>";
										echo "<td>";
										echo "<a href='?delete={$id}' onclick='return confirm(\"Are you sure you want to delete this entry?\")'> <i class='bx bx-message-square-x delete'></i> </a>"; // Delete icon
										echo "</td>";
										echo "</tr>";
									}
								} else {
									// If no messages are found, display this message
